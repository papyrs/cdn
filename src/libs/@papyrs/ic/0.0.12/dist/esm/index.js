...
var N=async({deck:e,fallbackAuthor:r})=>{let{data:o}=e,{meta:t,background:n,footer:i,header:s}=o;return {...await h({meta:t,selector:n$1,fallbackName:o.name,fallbackAuthor:r}),slides:D(),background:n?`<div slot="background">${n}</div>`:void 0,header:n?`<div slot="header">${s}</div>`:void 0,footer:n?`<div slot="footer">${i}</div>`:void 0}},v=async({doc:e,fallbackAuthor:r,theme:o$1})=>{let{data:t}=e,{meta:n}=t;return {...await h({meta:n,selector:o,fallbackName:t.name,fallbackAuthor:r}),paragraphs:E(),theme:o$1}},h=async({meta:e,fallbackName:r,fallbackAuthor:o,selector:t})=>{let n=L(),i=M({meta:e}),s=[n,i].filter(k=>k!==void 0),b=y({selector:t}),p=await H(),u=(e?.title||r)?.trim();return {title:u,description:(e?.description||r)?.trim(),author:e?.author?.name||o,bio:e?.author?.bio,photo_url:e?.author?.photo_url,head_extra:s.length>0?s.join(""):void 0,attributes:b,social_image_name:encodeURI(u).toLowerCase(),social_image_value:p}},L=()=>{let r=document.querySelector(n$1)?.style.fontFamily;if(!r)return;let o=e.find(n=>r===n.family.replace(/\'/g,""));return o?`<link rel="stylesheet" href="${n({font:o})}">`:void 0},M=({meta:e})=>{if(!(!e||!e.canonical))return `<link rel="canonical" href="${e.canonical}">`},y=({selector:e})=>{let o=document.querySelector(e)?.attributes;if(!!o)return Array.from(o).filter(({name:t})=>!["id","hydrated","class","contenteditable"].includes(t)).reduce((t,{name:n,value:i})=>(t[n]=i,t),{})},D=()=>{let e=document.querySelectorAll(`${n$1} > *[slide_id]`),r=({slide:t,custom:n})=>{if(t.hasAttribute(`custom-${n}`))return;let i=t.querySelector(`div[slot="${n}"]`);!i||t.removeChild(i);};return Array.from(e).map(t=>{let n=m({node:t,deep:!1,attributes:c});return Array.from(t.childNodes).forEach(s$1=>{s(s$1)&&s$1.hasAttribute("slot")&&(n.appendChild(m({node:s$1,attributes:c})),r({slide:n,custom:"background"}),r({slide:n,custom:"header"}),r({slide:n,custom:"footer"}));}),n}).map(t=>t.outerHTML)},E=()=>{let e=document.querySelectorAll(`${o} > article *[paragraph_id]`);return Array.from(e).map(o=>m({node:o,attributes:c})).map(o=>o.outerHTML)},H=async()=>document.querySelector("deckgo-social-img")?.toBlob("image/png");
...
const prepareIndexHtml = async ({ dataId, bucketUrl, publishData, metas }) => {
...
const updateList = async ({ dataId, template, bucketUrl, metas }) => {
  const links = metas.map((meta) => newLink({ dataId, meta, bucketUrl }));
...
const items = ({ metas, bucketUrl }) => metas.map(({ title, description, pathname, published_at }) => {
  var _a;
  return `
  <item>
...
const prepareSitemap = ({ bucketUrl, metas }) => {
  ...
  const postsUrls = metas.map(({ pathname }) => sitemapUrl(`${bucketUrl}${pathname}`));
  return siteMapTemplate([rootUrl, ...postsUrls].join(''));
};

const publishDeckMetas = async ({ owner_id, dataId, storageUpload, publishData }) => {
...
const publishDocMetas = async ({ owner_id, dataId, storageUpload, publishData }) => {
...
const mapMetas = (entries) => entries
  .filter(({ data }) => { var _a; return ((_a = data.meta) === null || _a === void 0 ? void 0 : _a.published) === true; })
  .map(({ data }) => data.meta)
  .sort(({ published_at: published_at_a }, { published_at: published_at_b }) => { var _a, _b; return (((_a = r(published_at_b)) === null || _a === void 0 ? void 0 : _a.getTime()) || 0) - (((_b = r(published_at_a)) === null || _b === void 0 ? void 0 : _b.getTime()) || 0); });
const publishMetas = async ({ dataId, storageUpload, publishData, entries }) => {
  const metas = mapMetas(entries);
  const promises = [
...
const publishIndexHtml = async ({ dataId, storageUpload, publishData, metas }) => {
  const { bucketUrl, actor } = storageUpload;
  const html = await prepareIndexHtml({ dataId, bucketUrl, publishData, metas });
...
const workerPromise = import('./sync.ic.worker-85088412.js').then(m => m.worker);
...
export { canistersBalance, createTemplate, deckEntries, deckPublish, deleteDeck, deleteDoc, deleteFile, docEntries, docPublish, getFiles, getParagraph, getSlide, getUserTemplates, publishUrl, snapshotDeck, snapshotDoc, sync, updateTemplate$1 as updateTemplate, updateUser, uploadFile, uploadFileIC };

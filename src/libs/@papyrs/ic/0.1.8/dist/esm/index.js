import { P as Principal } from './actor-da64ce67.js';
import { g as getDataBucket, a as getStorageBucket, c as createManagerActor, f as fromNullable, t as toNullable, b as fromArray, d as toArray, e as createActor, E as EnvStore, i as idlFactory$1 } from './auth.constants-795d1603.js';
import { g as getIdentity, t, K as KEY_STORAGE_KEY, a as KEY_STORAGE_DELEGATION, c as createWorkerProxy, i as isAuthenticated } from './auth.providers-029582fd.js';
export { e as deleteAuth, g as getIdentity, b as initAuth, i as isAuthenticated, d as signIn, s as signOut } from './auth.providers-029582fd.js';
import { u as update, g as get, s as set, c as createStore, a as getMany, i as isDelegationValid, D as DelegationChain } from './compat-7469423b.js';
...
const canistersControllers = async () => {
  var _a, _b;
  const identity = getIdentity();
  if (!identity) {
    throw new Error('No internet identity to get the canisters controllers');
  }
  const { getDataControllers, getStorageControllers } = await createManagerActor({
    identity
  });
  ...
  return {
    data: (_a = fromNullable(dataControllers)) !== null && _a !== void 0 ? _a : [],
    storage: (_b = fromNullable(storageControllers)) !== null && _b !== void 0 ? _b : []
  };
};
const addCanistersController = async (controller) => {
  const identity = getIdentity();
  if (!identity) {
    throw new Error('No internet identity to add the canisters controller');
  }
  const { setDataController, setStorageController } = await createManagerActor({
    identity
  });
  await Promise.all([
...
const deleteDeck = async (deckId, updated_at) => deleteData(Object.assign({ key: `/decks/${deckId}` }, (updated_at !== undefined && {
  data: { id: deckId, updated_at: updated_at }
})));
...
const deleteDoc = async (docId, updated_at) => deleteData(Object.assign({ key: `/docs/${docId}` }, (updated_at !== undefined && {
  data: { id: docId, updated_at: updated_at }
})));
// Backwards compatibility with current publish mask in studio and Firebase support. In case of IC we actually do not need a snapshot, publish is synchronous on the client side.
...
const updateTemplate = async ({ template, data, ids }) => {
  // 1. Replace all keys
  const updatedTemplate = [...Object.entries(data), ...Object.entries(ids)].reduce((acc, [key, value]) => acc
  ...
  const { data_id } = ids;
  const idsScript = `window.data_canister_id = "${data_canister_id}";window.storage_canister_id = "${storage_canister_id}";window.data_id = "${data_id !== null && data_id !== void 0 ? data_id : ''}";`;
  const templateWithScript = updatedTemplate.replaceAll('<!-- DECKDECKGO_IDS_SCRIPT -->', `<script>${idsScript}</script>`);
  // 3. Calculate a sha256 for the above script and parse it in the CSP
  const sha256 = sha256ToBase64String(new Uint8Array(await digestMessage(idsScript)));
  const templateWithCSP = templateWithScript.replaceAll('{{DECKDECKGO_IDS_SHAS}}', `'sha256-${sha256}'`);
...
const initUpload = async ({ indexHTML, folder, meta }) => {
  ...
  const bucketUrl = `https://${bucketId.toText()}.raw.icp0.io`;
...
const initIndexHTML = async ({ publishData, ids, updateTemplateContent, sourceFolder }) => {
...
const initDeckIndexHTML = async ({ deck, canisterIds }) => {
  ...
  const { html } = await initIndexHTML({
    publishData,
    ids: Object.assign(Object.assign({}, canisterIds), { data_id: deck.id }),
...
const initDocIndexHTML = async ({ doc, config, canisterIds }) => {
  ...
  const { html } = await initIndexHTML({
    publishData,
    ids: Object.assign(Object.assign({}, canisterIds), { data_id: doc.id }),
    updateTemplateContent,
    sourceFolder: 'd'
  });
  return {
...
const prepareIndexHtml = async ({ bucketUrl, publishData, metas, canisterIds }) => {
  ...
  let html = await updateTemplate({ template, data, ids: canisterIds });
...
const uploadResources = async ({ meta, ids }) => {
  // 1. Get actor
  const { actor } = await getStorageActor();
  ...
  const promises = kit.map((kit) => addKitIC({ kit, actor, meta, ids, assetKeys }));
  await Promise.all(promises);
};
...
const addDynamicKitIC = async ({ kit, actor, meta, ids, assetKeys }) => {
  const { src, filename, mimeType, updateContent, headers } = kit;
  const content = await downloadKit(src);
  const updatedContent = updateContent({ content, meta, ids: ids });
...
const addKitIC = async ({ kit, actor, meta, ids, assetKeys }) => {
  ...
  const content = await downloadKit(src);
  const updatedContent = updateContent ? updateContent({ content, meta, ids: ids }) : content;
  await uploadKit({
    filename,
    content: updatedContent,
    actor,
  ...
  const toResource = (resource) => {
    ...
    if (src.includes('hoisted.js')) {
      return {
        ...
        updateContent: ({ content, ids: { data_canister_id, data_id, storage_canister_id } }) => content
...
const getFiles = async ({ folder }) => {
  ...
  const host = `https://${bucketId.toText()}.raw.icp0.io`;
  return {
...
const workerPromise = import('./sync.ic.worker-7e591300.js').then(m => m.worker);

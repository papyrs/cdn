import { g as getDataBucket, a as getStorageBucket, t as toNullable, f as fromArray, b as fromNullable, c as toArray, d as createActor, E as EnvStore } from './auth.constants-98dfefd9.js';
import { g as getIdentity, t, c as createWorkerProxy, i as isAuthenticated } from './auth.providers-92ab2f5b.js';
export { d as deleteAuth, g as getIdentity, a as initAuth, i as isAuthenticated, b as signIn, s as signOut } from './auth.providers-92ab2f5b.js';
...
const uploadResources = async ({ meta }) => {
  ...
  const assetKeys = await actor.shas(toNullable('resources'));
  const keys = assetKeys.map(({ key: { fullPath }, sha256: sha256Array }) => {
    var _a;
    const sha256 = sha256ToBase64String(new Uint8Array((_a = fromNullable(sha256Array)) !== null && _a !== void 0 ? _a : []));
    return {
      fullPath,
  ...
  const promises = kit.map((kit) => addKitIC({ kit, actor, meta, assetKeys: keys }));
  await Promise.all(promises);
};
...
const workerPromise = import('./sync.ic.worker-52ef5718.js').then(m => m.worker);

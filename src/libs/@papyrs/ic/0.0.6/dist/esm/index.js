import { g as getIdentity, a as getDataBucket, b as getStorageBucket, f as fromNullable, c as fromArray, d as fromTimestamp, t as toArray, e as toTimestamp, h as toNullable, o as o$1, E as EnvStore, r as r$1, i as createWorkerProxy, j as internetIdentityAuth, k as isDelegationValid, D as DelegationChain } from './auth.providers-850d09a9.js';
export { n as deleteAuth, g as getIdentity, l as initAuth, m as signIn, s as signOut } from './auth.providers-850d09a9.js';
...
const publishOverview = async ({ dataId, storageUpload, publishData, meta }) => {
...
const updateList = async ({ dataId, template, storageUpload, publishData, meta }) => {
  const source = await htmlSource(storageUpload);
  const link = newLink({ dataId, meta, publishData, storageUpload });
  if (!source) {
    return template.replace(/<!-- DECKDECKGO_DATA -->/, `${link}`);
  }
  const matches = [...source.matchAll(/<a\x20.*?data-id=".*?"[\s\S]*?a>/gm)];
  if (!matches || matches.length <= 0) {
    return template.replace(/<!-- DECKDECKGO_DATA -->/, `${link}`);
  }
  const list = matches.map((match) => match[0]);
  const index = list.findIndex((a) => a.indexOf(dataId) > -1);
  if (index > -1) {
    return template.replace(/<!-- DECKDECKGO_DATA -->/, list.map((entry, i) => (i === index ? link : entry)).join(''));
  }
  return template.replace(/<!-- DECKDECKGO_DATA -->/, [link, ...list].join(''));
};
const newLink = ({ dataId, storageUpload, publishData, meta }) => {
  var _a, _b;
  ...
  const publishedAt = `<p>Published: ${format((_a = r$1(meta === null || meta === void 0 ? void 0 : meta.published_at)) !== null && _a !== void 0 ? _a : new Date())}</p>`;
  const editedAt = `<p>Edited: ${format((_b = r$1(meta === null || meta === void 0 ? void 0 : meta.updated_at)) !== null && _b !== void 0 ? _b : new Date())}</p>`;
...
const workerPromise = import('./sync.ic.worker-78d03eb6.js').then(m => m.worker);

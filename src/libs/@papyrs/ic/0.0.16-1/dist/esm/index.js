import { g as getIdentity, a as getDataBucket, b as getStorageBucket, f as fromNullable, c as fromArray, d as fromTimestamp, t as toArray, e as toTimestamp, h as toNullable, o as o$1, i as createActor, E as EnvStore, r, j as createWorkerProxy, k as internetIdentityAuth, l as isDelegationValid, D as DelegationChain } from './auth.providers-afa68723.js';
export { p as deleteAuth, g as getIdentity, m as initAuth, n as signIn, s as signOut } from './auth.providers-afa68723.js';
...
const docSubmitFeed = async ({ doc }) => {
  ...
  const updatedDoc = await updateMetaFeed({ key: 'docs', data: doc });
...
const deckSubmitFeed = async ({ deck }) => {
  ...
  await submitFeed({ meta, id });
  const updatedDeck = await updateMetaFeed({ key: 'decks', data: deck });
...
const submitFeed = async ({ meta, id }) => {
  ...
  const t1 = performance.now();
  o$1({ msg: '[submit][done] feed', duration: t1 - t0 });
};
const updateMetaFeed = async ({ key, data }) => {
  o$1({ msg: `[update][start] ${key}` });
  const t0 = performance.now();
  ...
  const t1 = performance.now();
  o$1({ msg: `[update][done] ${key}`, duration: t1 - t0 });
  return updatedData;
};
...
const workerPromise = import('./sync.ic.worker-fe088344.js').then(m => m.worker);

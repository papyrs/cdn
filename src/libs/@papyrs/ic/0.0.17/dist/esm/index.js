import { g as getIdentity, a as getDataBucket, b as getStorageBucket, t as toNullable, f as fromArray, c as fromNullable, d as toArray, e as t, h as createActor, E as EnvStore, i as createWorkerProxy, I as IdbStorage, j as isAuthenticated, k as isDelegationValid, D as DelegationChain } from './auth.providers-33f1da54.js';
export { n as deleteAuth, g as getIdentity, l as initAuth, j as isAuthenticated, m as signIn, s as signOut } from './auth.providers-33f1da54.js';
...
const setData$2 = async ({ key, idbData, actor = undefined }) => {
  const dataActor = actor || (await getDataActor());
...
const setData$1 = async ({ key, idbData, actor = undefined, log }) => {
  log === null || log === void 0 ? void 0 : log({ msg: `[set][start] ${key}`, level: 'info' });
  const t0 = performance.now();
  const updatedEntity = await setData$2({ key, actor, idbData: idbData });
...
const createTemplate = (data) => {
  ...
  return setData$1({ key: `/templates/${id}`, idbData: template });
};
...
const docSubmitFeed = async ({ doc }) => {
  ...
  const updatedDoc = await updateMetaFeed({ key: 'docs', idbData: doc });
...
const deckSubmitFeed = async ({ deck }) => {
  ...
  await submitFeed({ meta, id });
  const updatedDeck = await updateMetaFeed({ key: 'decks', idbData: deck });
...
const updateMetaFeed = async ({ key, idbData }) => {
...
const publishDeck = async ({ deck: deckSource }) => {
  ...
  const deck = await setData$1({
    key: `/decks/${id}`,
    idbData: Object.assign(Object.assign({}, deckSource), { data: deckData })
  });
  // 4. Upload
...
const publishDoc = async ({ doc: docSource, config }) => {
  ...
  const doc = await setData$1({
    key: `/docs/${id}`,
    idbData: Object.assign(Object.assign({}, docSource), { data: docData })
  });
  // 4. Upload
  await uploadPublishFileIC(storageUpload);
  // 5. Upload
...
const uploadResources = async ({ meta }) => {
  ...
  const kitNewFiles = kit.filter(({ filename, mimeType }) => !keys.includes(filename) || ['text/css', 'text/javascript'].includes(mimeType));
...
const workerPromise = import('./sync.ic.worker-f6024fc3.js').then(m => m.worker);

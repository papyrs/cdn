...
let DeckdeckgoLazyImg = class {
  constructor(hostRef) {
    ...
    this.lazyImgDidLoad = index.createEvent(this, "lazyImgDidLoad", 7);
    this.innerImgDidLoad = index.createEvent(this, "innerImgDidLoad", 7);
    this.customLoad = index.createEvent(this, "customLoad", 7);
    /**
  ...
  render() {
    ...
    if (this.svgContent) {
      return (index.h(index.Host, { class: hostClass }, index.h("div", { innerHTML: this.svgContent, class: "svg-container" })));
    }
    else {
      return index.h(index.Host, { class: hostClass }, this.renderImage());
    }
  }
  renderImage() {
    // prettier-ignore
    // @ts-ignore
    return index.h("img", { alt: this.imgLoaded ? (this.imgAlt ? this.imgAlt : this.imgSrc) : '', loading: this.loading, sizes: this.imgSizes ? this.imgSizes : undefined, intrinsicsize: this.intrinsicsize, width: this.imgWidth, height: this.imgHeight, onLoad: () => this.onImgDidLoad(), onError: () => this.loadError() });
  }
...
exports.deckgo_lazy_img = DeckdeckgoLazyImg;
